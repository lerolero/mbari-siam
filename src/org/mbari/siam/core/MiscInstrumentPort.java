...
/**
   Implements InstrumentPort interface for a miscellaneous port.
   May have a power port.
   @author Bob Herlien
 */
public class MiscInstrumentPort implements InstrumentPort
{
    private static String _versionID = "$Revision: 1.2 $";

    /** Log4j logger */
    protected static Logger _log4j = 
	Logger.getLogger(MiscInstrumentPort.class);
    ...
    public MiscInstrumentPort(String portName, PowerPort power, String params)
    {
	_portName = portName;
        _powerPort = power;
	_params = params;
    ...
    {
	throw new IOException("MiscInstrumentPort has no InstrumentConsole");
    }   
