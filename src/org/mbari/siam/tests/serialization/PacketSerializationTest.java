...
public class PacketSerializationTest{
	...
	/*
	SensorDataPacket2 sdp2=new SensorDataPacket2(1500,64);
	DeviceMessagePacket dmp=new DeviceMessagePacket(2000);
	MetadataPacket mdp=new MetadataPacket(3000,
					      "test012345".getBytes(),
					      "Metadata0123456".getBytes());
	SummaryPacket sp=new SummaryPacket(4000);
	*/

	/*
	sdp2.setSystemTime(now);
	sdp2.setSequenceNo(100L);
	sdp2.setMetadataRef(0L);
	sdp2.setParentId(parentID);
	sdp2.setRecordType(3L);
	sdp2.setDataBuffer("SensorData2_012".getBytes());
	...
	*/
	try{
	    ...
	    ExternalizableSensorDataPacket esdp=new ExternalizableSensorDataPacket(exID,64);
	    ...
	    DeviceLog dlESDP=new DeviceLog(exID,"./");
...
*/
		...
		System.out.println("packet "+ (i++) +":"+(ExternalizableSensorDataPacket)e.nextElement());
*/
