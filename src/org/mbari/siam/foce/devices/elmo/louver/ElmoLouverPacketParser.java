...
public class ElmoLouverPacketParser extends PacketParser {

    static private Logger _log4j = Logger.getLogger(ElmoLouverPacketParser.class);

    private static final long serialVersionUID=1L;
	public ElmoLouverPacketParser(){
		super();
	}
    public ElmoLouverPacketParser(String registryName){
	super(registryName);
    }
	...
	PacketParser.Field[] fields = new PacketParser.Field[18];
	try{
	    fields[0]=new PacketParser.Field("name",this._registryName,"string");
	    // data format: status,velocity,faults
	    fields[1] = new PacketParser.Field("louverCommand",new Double(st.nextToken()),"percent");
	    fields[2] = new PacketParser.Field("louverPosition",new Double(st.nextToken()),"percent");
	    fields[3] = new PacketParser.Field("positionCounter",new Long(st.nextToken()),"counts");
	    Integer hallPhysical=new Integer(Integer.parseInt(st.nextToken(),16));
	    fields[4] = new PacketParser.Field("hallPhysical",hallPhysical,"integer");
	    Integer hallLogical=new Integer(Integer.parseInt(st.nextToken(),16));
	    fields[5] = new PacketParser.Field("hallLogical",hallLogical,"integer");
		Long posError=new Long(st.nextToken());
	    fields[6] = new PacketParser.Field("positionError",posError,"counts");
		Integer status=new Integer(Integer.parseInt(st.nextToken(),16));
	    fields[7] = new PacketParser.Field("statusRegister",status,"bit field (int)");
	    Long fault=new Long(Long.parseLong(st.nextToken(),16));
	    fields[8] = new PacketParser.Field("faultRegister",fault,"bit field (long)");
	    fields[9] = new PacketParser.Field("unitMode",new Integer(st.nextToken()),"integer");
		for(int i=0;i<6;i++){
	      fields[10+i] = new PacketParser.Field(("inputLogic."+(i+1)),new Integer(st.nextToken()),"integer");
		}
	    fields[16] = new PacketParser.Field("srString","\n"+ElmoLouverPacketParser.getStatusString(status),"string");
	    fields[17] = new PacketParser.Field("mfString","\n"+ElmoLouverPacketParser.getFailureString(fault),"string");
