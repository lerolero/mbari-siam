...
public class RemoveSchedule {
	...
	RemoveSchedule removeSchedule = new RemoveSchedule();
	...
	if (args.length != 3) {
	    System.err.println("Usage: nodeURL port scheduleName");
	    System.exit(1);
	}
	...
	try {
	    byte[] stat=nodeService.removeSchedule(port.getBytes(),schedule.getBytes());
	    System.out.println("RemoveSchedule "+port+":"+schedule+" returned "+new String(stat));
	}
