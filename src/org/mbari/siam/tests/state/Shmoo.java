    ...
    extends InstrumentService implements Instrument 
{
    ...
    public Shmoo() throws RemoteException 
    {
    ...
    protected void initializeInstrument() throws InitializeException, Exception
    {
        ...
        {
            DebugMessage.println("ShmooInstrumentService has " + 
    ...
    {
    	DebugMessage.println(new String(getName()) + 
                             ".requestSample(): entry");
        _fromDevice.flush();
        _toDevice.write(SHMOO_REQUEST);
    ...
    */
	...
	_sensorDataPacket.setSystemTime(System.currentTimeMillis());
        ...
        int randomDelay = 0;
        if ( _attributes.snarbStarm > 0 )
        {
            //the forbidden dance
            Random rand_a_lor = new Random(System.currentTimeMillis());
            randomDelay = rand_a_lor.nextInt(_attributes.snarbStarm);
            StopWatch.delay(randomDelay);
        }
        
        //measure current and voltage
        DebugMessage.println("channel voltage: " + 
                             _instrumentPort.getVoltageLevel());
        DebugMessage.println("channel current: " + 
