...
public class EnvironmentalPacketParser extends SensorPacketParser {
	...
	while(st.hasMoreTokens()){
		...
		_fields.setElementAt(getDouble(token,3),GFLO);
	    if(token.toUpperCase().startsWith("GFH"))
		_fields.setElementAt(getDouble(token,3),GFHI);
	}
	return _fields;
    }

    public Object  get(int field){
	//System.out.println("EnvironmentalPacketParser fields: "+_fields.size()+" {"+_fields+"} getting field:"+field);
