#!/bin/bash
# set up linux JAVA build environment for SIAM export
...
# set JAVA_HOME
# e.g., /System/Library/Frameworks/JavaVM.framework/Versions/Current/Commands/java_home
export JAVA_HOME="$2"
...
if [ "$addPath" -ne 0 ]
  then
  export PATH=$SIAM_HOME/utils${SEP}$PATH
fi
...
if [ "$addPath" -ne 0 ]
  then
  export PATH=$SIAM_HOME/utils/foce${SEP}$PATH
fi
