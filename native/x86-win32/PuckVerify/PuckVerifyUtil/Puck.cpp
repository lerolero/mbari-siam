...
//funcs for writing
int Puck::startWriting(long address)
{
	int retVal;

	if ( _readEnabled )
		return ERR_READ_ENABLED;
	...
	retVal = sendPuckCmd("PUCKEM\r", NULL, 3, 10000); 

    if ( retVal )
    {
		statMsg("ERR: PUCKEM command failed while ");
		statMsg("enabling a write session\r\n");
		return retVal;
	}
	...
	statMsg("STATUS: PUCK write session enabled\r\n");
...
int Puck::endWriting()
{
	...
	//write any bytes that are buffered but not sent
	while (_writeBufferIndex < WRITE_BUFFER_SIZE)
	{
		retVal = write((char)0xff);
		if ( retVal )
		{
			_writeEnabled = false;
			return retVal;
    ...
    {
		_writeEnabled = false;
		statMsg("ERR: failed to write PUCK data packet, ");
		statMsg("write session may be incomplete\r\n");
		return retVal;
	}

	retVal = sendPuckCmd("PUCKFM\r", NULL, 3, 1000); 

	_writeEnabled = false;

    if ( retVal )
    {
		statMsg("ERR: PUCKFM command failed, bytes may not be flushed\r\n");
		return retVal;
	}
...
int Puck::write(char c)
{
	int retVal;

	if ( !_writeEnabled )
	{
		statMsg("ERR: PUCK write failed, writing was not enabled\r\n");
		return ERR_WRITE_DISABLED;
	}

	if ( _writeBufferIndex < WRITE_BUFFER_SIZE )
	{
	...
	else
	{
		...
		if ( retVal )
		{
			statMsg("ERR: PUCK write failed while writing a packet\r\n");
			_writeEnabled = false;
			return retVal;
		}
		...
		//store the next byte in the buffer position
		_writeBuffer[_writeBufferIndex++] = c;
	}
...
int Puck::writePacket()
{
	char cmd[16];
	int retVal;
	int bytesRead;

	sprintf(cmd, "PUCKWM %d\r", WRITE_BUFFER_SIZE);

	//send the write command
	_serialPort.Write(cmd, strlen(cmd));

	//send out data
	_serialPort.Write(_writeBuffer, WRITE_BUFFER_SIZE);

    //check for the RDY prompt
	retVal = readUntil(NULL, 1000, 1000, &bytesRead);

	if ( retVal )
	{
		statMsg("ERR: PUCK write failed on PUCKWM command\r\n");
		_writeEnabled = false;
		return retVal;
	}
...
//funcs for reading
int Puck::startReading(long address)
{
		return ERR_FAILURE;
}
...
int Puck::readAvailable()
{
	return ERR_FAILURE;
}
    
int Puck::read(char& c)
{
	return ERR_FAILURE;
}
